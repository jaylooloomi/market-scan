...
3. 直接把 Reviewer 的 verdict 彙整成中文 markdown 四區報告(**plugin 模式不需跑任何 python**,全靠 subagent + MCP 工具)。
   ...
   `python -c "from polydig_mcp.reviewer.pipeline import run_daily; print(run_daily()['report_md'])"`,報告開頭會含「今日大盤風險」橫幅。）
...
3. 用 `mcp__polydig-data__get_history_match` 取歷史對應(讀內建題材庫,免工作目錄)。
