# PolyDig — Design Spec v0.1

**Date**: 2026-05-31
**Author**: Arthur (jaylooloomi) + Claude (Haiku 4.5)
**Status**: Draft，等用戶 review
**GitHub**: https://github.com/jaylooloomi/polydig
**Working dir**: `D:\git\harness-run\polydig\`

---

## 1. 專案一句話定義

> **PolyDig 是一套台股題材早期偵測系統，在事件還沒被市場廣泛察覺時，把「事件 → 受影響族群 → 歷史對應」這個推理鏈跑出來，輸出每日研究助理報告。**
...
### 7.1 為什麼這是 Phase 0
驗證 PolyDig 的根本假設：**「訊號出現時，股價真的還沒漲」**。

如果這個假設不成立（事件出現時股票已經漲了一波），整個 PolyDig 沒價值。
...
| 8 | Git 設定 | **立即 git init + push** 到 [polydig repo](https://github.com/jaylooloomi/polydig) |
...
```bash
# 跑單一 case
$ polydig-validator --case mask --triggers early,mid,late

# 跑全部 cases
$ polydig-validator --config cases.json --output reports/
...
5. **Git 與 GitHub**：Phase 0 開始時要不要 `git init` + push 到 [polydig repo](https://github.com/jaylooloomi/polydig)？
