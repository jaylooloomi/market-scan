"""`market-scan-daily` — run one daily cycle headlessly and write a markdown report.

Used for OS cron / scheduled runs outside an interactive Claude Code session.
Inside Claude Code, the market-scan-daily skill drives the same flow via subagents.
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Market Scan daily research report")
    ...
    print(f"Market Scan daily run — RAG mode: {store.mode}, {mode_label}", file=sys.stderr)
    ...
    if args.telegram:
        from market_scan_mcp.reporting.telegram import send_message
        header = f"📊 Market Scan 每日台股題材報告 {date.today().isoformat()}\n\n"
