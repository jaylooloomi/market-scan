...
`polydig-daily --db ./polydig.db` daily over time — that part cannot be fast-forwarded.
...
production (e.g. reuse polydig_validator.excess_return).
"""
...
def signal_volume(verdicts: list[dict[str, Any]], noise_target: int = 5) -> dict[str, Any]:
    ...
    `verdicts` is the output of PolyDigDB.query_verdicts (each has date + grade).
    """
...
def compute_metrics(
    ...
    **hit_kwargs: Any,
    ...
    """Compute the spec §10 dashboard from a PolyDigDB. `signal_volume` always runs;
...
def finmind_forward_return(
    ...
    `query` is injectable for testing; defaults to polydig_mcp.data.finmind.query
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="PolyDig spec §10 operational metrics (from SQLite)")
    p.add_argument("--db", default="polydig.db", help="SQLite db written by polydig-daily --db")
